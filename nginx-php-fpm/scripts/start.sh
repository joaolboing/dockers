  ...
  if [ -f $XdebugFile ]; then
    ...
    if [ -f $XdebugFile ]; then
        ...
        if grep -q xdebug.remote_enable "$XdebugFile"; then
            ...
            echo "xdebug.remote_host=docker.local" >> $XdebugFile
            echo "xdebug.remote_port=9000" >> $XdebugFile
            echo "xdebug.var_display_max_depth=-1" >> $XdebugFile
            echo "xdebug.var_display_max_children=-1" >> $XdebugFile
            echo "xdebug.var_display_max_data=-1" >> $XdebugFile
